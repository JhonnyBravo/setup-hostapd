...
default_path="/etc/network/interfaces"
template_path="/usr/share/doc/ifupdown/examples/network-interfaces.gz"
...
t_description=$(
  cat <<_EOT_
  -t directory_path   ${template_path} を
                      directory_path へコピーします。
_EOT_
)
...
if [ $g_flag -eq 1 ]; then
  ...
  directory_name="$1"
  zcat "$template_path" > "${directory_name}/${file_name}"
